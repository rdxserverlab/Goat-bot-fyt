 process.stdout.write("\x1b]2;BAYJID BOT - Made by MOHAMMAD BAYJID ");
...
let subTitle = `BAYJID BOT@${currentVersion}- A simple Bot chat messenger use personal account`;
const subTitleArray = [];
...
const srcUrl = ("Source code: https://github.com/BAYJID-404-BOYZ");
	...
	try {
		...
		// is cookie string
			...
			// is json (cookies or appstate)
				...
				appState = appStateGithub.com/BAYJID-404-BOYA
					.map(item => ({
						...item,
						domain: "facebook.com",
						path: "/",
...
async function startBot(loginWithEmail) {
	...
	(function loginBot(appState) {
		...
		login({ appState }, global.GoatBot.config.optionsFca, async function (error, api) {
			...
			log.info("BAYJID࿐ツ NODE VERSION", process.version);
			log.info("BAYJID࿐ツ PROJECT VERSION", currentVersion);
			log.info("BAYJID࿐ツ BOT ID", `${global.botID} - ${await getName(global.botID)}`);
			log.info("BAYJID࿐ツ PREFIX", global.GoatBot.config.prefix);
			log.info("BAYJID࿐ツ LANGUAGE", global.GoatBot.config.language);
			log.info("BAYJID࿐ツ BOT NICK NAME", global.GoatBot.config.nickNameBot || "ARYAN BOT");
			// ———————————————————— GBAN ————————————————————— //
			let dataGban;
			...
			let notification;
			try {
				const getNoti = await axios.get("https://raw.githubusercontent.com/BAYJID-404-BOYZ/PUBLIC-/refs/heads/main/notification.txt");
					...
					if (filename.endsWith('.js')) {
						...
						if ((event == 'change' || event == 'rename') && existsSync(`${process.cwd()}/scripts/events/${filename}`)) {
							try {
								...
								log.err("BAYJID AUTO LOAD SCRIPTS", `Error when reload event ${filename}.js`, err);
							}
						}
					}
				});
			...
			log.master("BAYJID࿐ツ NOTIFICATION", (notification || "").trim());
			log.master("BAYJID࿐ツ SUCCESS", getText('login', 'runBot'));
			log.master("BAYJID࿐ツ LOAD TIME", `${convertTime(Date.now() - global.GoatBot.startTime)}`);
			...
			console.log(`\x1b[1m\x1b[33m${("COPYRIGHT:")}\x1b[0m\x1b[1m\x1b[37m \x1b[0m\x1b[1m\x1b[36m${("Project BAYJID BOT created by BAYJID࿐ツ (https://github.com/BAYJID-404-BOYZ), please do not sell this source code or claim it as your own. Thank you!")}\x1b[0m`);
			logColor("#f5ab00", character);
